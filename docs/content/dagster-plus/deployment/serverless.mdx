...
- Some features of Serverless deployments needs to be replaced with Hybrid equivalent:
  ...
  - [Non-isolated runs](/dagster-plus/deployment/serverless#non-isolated-runs) have no direct equivalent in Hybrid deployments. However, you can explore using the [`in_process_executor` or `multiprocess_executor`](/_apidocs/execution#executors) for specific jobs or for whole code locations to reduce the overhead.
